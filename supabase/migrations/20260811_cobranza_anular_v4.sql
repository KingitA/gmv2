-- ============================================================================
-- cobranza_anular v4 — anular un pago revierte TODO lo que el pago generó,
-- incluidos los AJUSTES POR REDONDEO
--
-- v4 = v3 + paso 1c: los ajustes manuales nacidos del cobro llevan la marca
-- [pago:<id>] en observaciones (y [saldo:<comprobante_id>] si además saldaron
-- un comprobante). Al anular el pago:
--   - se postea el CONTRA-ASIENTO exacto del ajuste (con marca [reversa:<id>],
--     misma que usa el 🗑 de la cta cte — imposible revertir dos veces), y
--   - si el ajuste había saldado un comprobante, ese saldo se restaura.
-- Sin esto, anular un cobro con "ajuste por redondeo" dejaba el crédito del
-- ajuste vivo y el comprobante saldado de más: descuadre igual al ajuste.
--
-- (Hereda de v3: bonificaciones por vínculo estructural — REV interna se
-- revierte completa; NC fiscal con CAE se informa para emitir ND.)
-- ============================================================================
