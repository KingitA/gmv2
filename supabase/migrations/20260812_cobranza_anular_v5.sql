-- ============================================================================
-- cobranza_anular v5 — la anulación también destilda "cobrado" en el KARDEX
--
-- v5 = v4 + paso 7b: el kardex de mercadería es la fuente de verdad de los
-- hechos del artículo (lo leen el módulo de vendedores y el Playroom). La
-- post-confirmación tilda kardex.comprobante_cobrado al saldarse cada
-- comprobante; esta versión lo DESTILDA cuando la anulación del pago deja el
-- comprobante sin pagar — si no, la mercadería figuraría cobrada para siempre.
--
-- (Hereda de v4: reversa de ajustes por redondeo vinculados [pago:]/[saldo:];
-- de v3: bonificaciones por vínculo estructural — REV interna se revierte
-- completa; NC fiscal con CAE se informa para emitir ND.)
-- ============================================================================
...
-- ── Backfill: tildar "cobrado" en el kardex de los comprobantes que YA están
--    pagados (cobros hechos antes de este fix, ej. Urquiza 12/08) ──
UPDATE kardex k
SET comprobante_cobrado = true, fecha_comprobante_cobrado = now()
FROM comprobantes_venta cv
WHERE cv.id = k.comprobante_venta_id
  AND cv.estado_pago = 'pagado'
  AND cv.anulado_en IS NULL
  AND k.comprobante_cobrado = false;
