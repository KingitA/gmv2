...
export function MoverPlata({
  ...
  const [modo, setModo] = useState<"transferencia" | "egreso">("transferencia")
  ...
  const mover = async () => {
    ...
    try {
      ...
      toast({
        title: modo === "transferencia" ? "Plata movida" : "Egreso registrado",
        description:
    ...
    <div className="mb-4 rounded-xl border-2 border-slate-400 bg-white px-3.5 py-2.5">
      <div className="flex flex-wrap items-center gap-2.5">
        <div className="inline-flex rounded-lg bg-slate-200 p-0.5">
          {(["transferencia", "egreso"] as const).map((m) => (
            <button
              key={m}
              onClick={() => setModo(m)}
            ...
            >
              {m === "transferencia" ? "⇄ Entre cuentas" : "↓ Egreso / gasto"}
            </button>
          ))}
        </div>
