...
function SwipeItem({ item, onConfirmFaltante, saving, bgReveal, colorReveal, labelReveal, iconReveal }:
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    if (diff > 0) setDx(Math.min(diff, THRESHOLD + 10))
    else setDx(0)
  }
  const onTouchEnd = () => {
  ...
  const handleRevealClick = () => {
    onConfirmFaltante(item)
    setRevealed(false); setDx(0)
  }
    ...
    <div style={{ position: "relative", overflow: "hidden", borderRadius: 16, marginBottom: 8 }}>
      ...
      <div onClick={handleCardClick}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: C.text, fontWeight: 700, fontSize: 20, lineHeight: 1.25, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", marginTop:5, flexWrap:"wrap" }}>
            <span style={{ color:C.light, fontSize:14, fontFamily:"monospace" }}>{item.articulos?.sku}</span>
            {item.articulos?.proveedores?.nombre && (
              <span style={{ color:C.orange, fontSize:14, fontWeight:600 }}>{item.articulos.proveedores.nombre}</span>
            )}
            {item.es_bonificado && (
              <span style={{ color:"#b45309", fontSize:13, fontWeight:600 }}>Bonificado — agregar como ítem normal</span>
            )}
          </div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          {ok ? <>
            <div style={{ color: C.green, fontWeight: 800, fontSize: 24 }}>{item.cantidad_preparada}</div>
            <div style={{ color: C.light, fontSize: 14 }}>de {item.cantidad}</div>
          </> : faltante ? (
            <div style={{ color: C.red, fontWeight: 700, fontSize: 15 }}>FALTANTE</div>
          ) : <>
            <div style={{ color: C.text, fontWeight: 800, fontSize: 24 }}>{item.cantidad}</div>
...
export default function PickingPage() {
  ...
  const marcarFaltante = async (item: DetallePedido) => {
    setSaving(true)
    try {
      const r = await fetch("/api/deposito/picking/item", {
        method:"PATCH", headers:{"Content-Type":"application/json"},
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100dvh - 64px)", background:C.bg }}>
      ...
      <div style={{ flex:1, overflow:"auto", padding:"0 16px", display:"flex", flexDirection:"column", gap:10 }}>
        {resultados.map(art => {
          ...
          const listo = enPedido?.estado_item === "COMPLETO" || enPedido?.estado_item === "PARCIAL"
          return (
  ...
  if (vistaFaltantes) return (
    <div style={{ background:C.bg, minHeight:"calc(100dvh - 64px)", padding:"16px 14px" }}>
      ...
      <div style={{ marginBottom:18 }}>
        ...
        <div style={{ fontSize:14, color:C.sub, marginTop:4 }}>Swipeá hacia la derecha para devolver a pendientes</div>
      </div>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          ...
          {faltantesList.length > 0 && (
            <button onClick={() => setVistaFaltantes(true)}
              style={{ background:C.redL, border:`1.5px solid ${C.redB}`, borderRadius:12, padding:"7px 14px", fontSize:13, fontWeight:700, color:C.red, cursor:"pointer", whiteSpace:"nowrap" }}>
