    ...
    <div style={{ position: "relative", overflow: "hidden", borderRadius: 16, marginBottom: 8 }}>
      ...
      <div onClick={handleCardClick}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", marginTop:5, flexWrap:"wrap" }}>
            ...
            {item.articulos?.proveedores?.nombre && (
              <span style={{ color:C.orange, fontSize:15, fontWeight:600 }}>{item.articulos.proveedores.nombre}</span>
            )}
            {item.es_bonificado && (
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          {ok ? <>
            <div style={{ color: C.green, fontWeight: 800, fontSize: 27 }}>{item.cantidad_preparada}</div>
            <div style={{ color: C.light, fontSize: 15 }}>de {item.cantidad}</div>
          </> : faltante ? (
            <div style={{ color: C.red, fontWeight: 700, fontSize: 16 }}>FALTANTE</div>
          ) : <>
            <div style={{ color: C.text, fontWeight: 800, fontSize: 27 }}>{item.cantidad}</div>
