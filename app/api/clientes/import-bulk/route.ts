...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { connector, rows, dry_run } = body as {
      connector: string
    ...
    for (const row of rows) {
      ...
      if (!connVal) {
        filas.push({ codigo_cliente: null, cuit: null, nombre: null, status: "error", cambios: [], error: "Fila sin valor en la columna conectora." })
        continue
      }
      ...
      if (!existing) {
        filas.push({
          codigo_cliente: esCodigo ? connVal : null,
          cuit: esCodigo ? null : connVal,
          nombre: null,
      ...
      const baseInfo = {
        codigo_cliente: existing.codigo_cliente ?? null,
