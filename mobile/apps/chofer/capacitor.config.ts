...
const config: CapacitorConfig = {
  ...
  android: {
    ...
    // Solo para probar contra un Next/mock local por http (GM_DEV_HTTP=1 npx cap sync).
    ...
    // instalado sin borrarle los datos (build-apks.mjs --devtools). Nunca en un release.
    webContentsDebuggingEnabled: process.env.GM_DEBUG_WEBVIEW === "1" || process.env.GM_DEV_HTTP === "1",
  },
  server: {
    androidScheme: "https",
